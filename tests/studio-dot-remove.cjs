...
(async()=>{
 ...
 async function install(){
  await page.evaluate(({css,drawing})=>{
   ...
   window.state=saved?JSON.parse(saved):{currentProject:'job',currentCabinet:'cab',currentPart:null,selectedCopy:-1,projects:[{id:'job',name:'Kitchen',rooms:[],cabinets:[{id:'cab',name:'Unit',drawing,parts:[
    {id:'a',code:'P-001',name:'Side',length:720,width:560,thickness:19,qty:1,status:'ready',x:25,y:25,copies:[]},
    {id:'b',code:'P-002',name:'Shelf',length:500,width:300,thickness:19,qty:2,status:'ready',x:75,y:75,copies:[{x:60,y:40}]}]}]}]};
 ...
 console.log('PASS: dot click without new panel, drag, Remove dot keeps panel and quantity, reload, re-place main and repeated dots, normal new panel click.');
