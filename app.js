...
function renderCustomerDetail(name,entry){
 ...
 if(jobList)jobList.querySelectorAll("[data-open-customer-project]").forEach(b=>b.onclick=()=>{
  ...
  const p=project();
  // Opening a job from a real customer's card means you've already told us
  // who this is -- carry that name onto the project itself so Save to
  // Customer on the Drawing screen never has to ask again.
  if(p&&!String(p.customer||'').trim()&&!/^No customer name/.test(name))p.customer=name;
...
(function(){
  ...
  function finalizeSaveToCustomer(p,name){
    ...
    // mergeMobileSiteJob) -- naming the job here should stick for all of
  ...
  // Picking an existing customer is one tap; typing a name is only ever
  // needed for a genuinely new customer, kept as a deliberate secondary
  // action rather than the default.
  function showSaveToCustomerPicker(p){
    const existing=[...(typeof allCustomerEntries==='function'?allCustomerEntries():new Map()).keys()].filter(k=>!/^No customer name/.test(k)).sort((a,b)=>a.localeCompare(b));
    const overlay=document.createElement('div');
    overlay.className='project-notes-overlay';
    const listHtml=existing.length?existing.map(n=>`<button type="button" class="btn save-to-customer-pick" data-pick="${safe(n)}">${safe(n)}</button>`).join(''):`<div class="muted">${st('common.noCustomersYet')}</div>`;
    overlay.innerHTML=`<div class="project-notes-sheet" role="dialog" aria-modal="true" aria-labelledby="saveToCustomerTitle"><div class="project-notes-head"><span class="notes-big-icon">👤</span><h2 id="saveToCustomerTitle">${st('drawing.pickCustomerTitle')}</h2></div><div class="save-to-customer-list" style="display:flex;flex-direction:column;gap:8px;max-height:50vh;overflow:auto;margin:14px 0">${listHtml}</div><div class="project-notes-actions"><button type="button" class="btn" data-close-picker>${st('common.cancelBtn')}</button><button type="button" class="btn primary" data-new-customer>${st('drawing.newCustomerBtn')}</button></div></div>`;
    document.body.appendChild(overlay);
    const close=()=>overlay.remove();
    overlay.addEventListener('click',e=>{if(e.target===overlay)close()});
    overlay.querySelector('[data-close-picker]').onclick=close;
    overlay.querySelectorAll('[data-pick]').forEach(b=>b.onclick=()=>{finalizeSaveToCustomer(p,b.dataset.pick);close()});
    overlay.querySelector('[data-new-customer]').onclick=()=>{
      close();
      const name=(prompt(st('drawing.enterCustomerNamePrompt'))||'').trim();
      if(!name)return;
      finalizeSaveToCustomer(p,name);
    };
  }
  document.addEventListener('click',function(e){
    ...
    if(name){finalizeSaveToCustomer(p,name);return}
    showSaveToCustomerPicker(p);
  },true);
})();
