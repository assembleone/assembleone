...
const statuses=[["ready","Ready"],["installed","Installed"],["missing","Missing"],["damaged","Damaged"]];
...
(function(){
 ...
 function updateHome(){
   ...
   if(t)t.textContent=p?(p.customer?`${p.customer} — ${p.name||'Current job'}`:(p.name||'Current job')):'Open the job you are working on';
 }
 ...
 document.addEventListener('DOMContentLoaded',function(){
   ...
   q('continueCurrentBtn')?.addEventListener('click',function(){const p=currentProject();if(!p){document.getElementById('a100StartShell')?.scrollIntoView({behavior:'smooth',block:'start'});return}showSetup()});
   ...
   q('settingsClearDataBtn')?.addEventListener('click',function(){
     if(!confirm('This removes every job, drawing and setting saved on this device. This cannot be undone. Continue?'))return;
