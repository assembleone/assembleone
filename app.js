...
function renderRooms(){
 ...
 sizeCanvas();
 canvas.querySelectorAll(".pin").forEach(x=>x.remove());canvas.querySelectorAll(".site-measure-overlay").forEach(x=>x.remove());img.style.display=pdf.style.display="none";empty.style.display="block";$("#drawingName").textContent=c?.drawingName||c?.name||st('drawing.noDrawing');
...
function renderCustomerDetail(name,entry){
 ...
 if(jobList)jobList.innerHTML=projects.length?projects.map(p=>{
  ...
  return `<div class="customer-job-card" draggable="true" data-drag-job="${safe(p.id)}" title="${st('common.dragToTradeDbTitle')}">${photo?`<img class="customer-job-photo" src="${photo}">`:`<div class="customer-job-photo customer-job-photo-empty">${safe(r.icon||"🏠")}</div>`}<div class="customer-job-info"><strong>${safe(r.icon||"🏠")} ${safe(p.name||r.name||st('common.roomFallback'))}</strong><span class="customer-job-meta"><span>📷 ${photos}</span><span>📏 ${measures}</span>${panels?`<span>🧩 ${panels}</span>`:''}</span>${sibTag}</div><div class="customer-job-actions">${photos?`<button class="btn site-details-btn" type="button" data-customer-photos="${safe(p.id)}">${st('common.viewPhotosBtn')}</button>`:''}<button class="btn primary" type="button" data-open-customer-project="${safe(p.id)}">${st('common.openArrowBtn')}</button><button class="btn danger" type="button" data-delete-customer-project="${safe(p.id)}">${st('common.delete')}</button></div></div>`;
 ...
 if(jobList)jobList.querySelectorAll("[data-customer-photos]").forEach(b=>b.onclick=e=>{
  e.stopPropagation();
  openSiteRoomDetails(b.dataset.customerPhotos);
 });
...
(function(){
  ...
  // file the current drawing, rather than trusting the continuous background
  // autosave, with a clear confirmation and a trip back to the job list
  // (Home) it was opened from. If the job already has a customer name on
  // file, that also carries it to sibling rooms from the same phone site
  // visit (see mergeMobileSiteJob and allCustomerEntries) -- but a job
  // that's already open never needs one to be saved.
  ...
  document.addEventListener('click',function(e){
    ...
    const p=typeof project==='function'?project():null;
    if(!p)return;
    finalizeSaveToCustomer(p,String(p.customer||'').trim());
  },true);
})();
