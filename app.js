...
function renderCustomers(){
 ...
 if(!entries.length){list.innerHTML=`<div class="empty">${map.size?"No customers match your search.":'No customers yet. Tap "+ New customer" to add one, or send a Site Job from your phone.'}</div>`;return}
 list.innerHTML=entries.map(([name,e])=>{
  const jobCount=e.projects.length;
  return `<button class="customer-row" type="button" data-customer="${safe(name)}"><span class="customer-row-info"><strong>${safe(name)}</strong>${e.address?`<span class="customer-row-address">${safe(e.address)}</span>`:""}</span>${jobCount?`<span class="customer-row-count">${jobCount} job${jobCount===1?"":"s"}</span>`:""}<span class="customer-row-arrow">›</span></button>`;
 }).join("");
...
async function saveSitePacketToCustomer(syncId){
 try{
  ...
  if(!pack)return alert('This site job is no longer waiting.');
  ...
  alert(`Saved to ${merged.customer||'the customer'}'s jobs — find it anytime from My Projects.`);
 }catch(e){
  console.error(e);
  alert('Could not save this job. Please try Open Drawing instead.');
 }
}
...
function ensureSiteReferenceCabinet(merged){
 ...
 if(pics[0]){c.drawing=pics[0];c.drawingType='image';c.drawingName=(room?.name||'Site room')+' site measurements';c.siteReference=true;c.beforePicture=pics[0];c.siteMarkup=room?.siteMarkup?JSON.parse(JSON.stringify(room.siteMarkup)):null}
 return c;
}
